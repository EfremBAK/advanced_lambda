//instead of for specific type, use a generic type so that code can be applied broadly
...
public class App9 {
    public static void main(String[] args) {
        ...
        int count = personProcess(roster,
    ...
    public static <X, Y>int personProcess(Collection<X> source, Predicate<X> tester, Function<X, Y> mapper, Consumer<Y> block){
        int count = 0;
        for(X p: source){
            if(tester.test(p)){
                ++count;
 ...
 *
 * 1) Obtains a source of objects from the collection source. In this example, it obtains a source of Person objects
 * from the collection roster. Notice that the collection roster, which is a collection of type List, is also an object
 * of type Iterable.
 *
